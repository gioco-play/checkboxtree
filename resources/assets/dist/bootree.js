...
bt.dialog.methods = {
    ...
    localization: function($dialog) {
        ...
        if (typeof (data.title) === 'undefined') {
            data.title = bt.dialog.messages['en-us'].DefaultTitle;
        }
    },
    ...
    renderHeader: function ($dialog) {
        ...
        if ($closeButton.length === 0 && data.closeButtonInHeader) {
            $closeButton = $('<button type="button" data-role="close" title="' + bt.dialog.messages['en-us'].Close + '"><span>×</span></button>');
...
bt.grid.methods = {
    ...
    localization: function (data) {
        if (!data.notFoundText) {
            data.notFoundText = bt.grid.messages['en-us'].NoRecordsFound;
        }
    },
    ...
    startLoading: function ($grid) {
        ...
        $loading = $('<div data-role="loading-text">' + bt.grid.messages['en-us'].Loading + '</div>').addClass(data.style.loadingText);
...
bt.grid.plugins.inlineEditing.private = {
    localization: function (data) {
        if (data.uiLibrary === 'bootstrap') {
            data.inlineEditing.editButton = '<button role="edit" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span> ' + bt.grid.messages['en-us'].Edit + '</button>';
            data.inlineEditing.deleteButton = '<button role="delete" class="btn btn-default btn-sm bt-margin-left-10"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span> ' + bt.grid.messages['en-us'].Delete + '</button>';
            data.inlineEditing.updateButton = '<button role="update" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-ok" aria-hidden="true"></span> ' + bt.grid.messages['en-us'].Update + '</button>';
            data.inlineEditing.cancelButton = '<button role="cancel" class="btn btn-default btn-sm bt-margin-left-10"><span class="glyphicon glyphicon-ban-circle" aria-hidden="true"></span> ' + bt.grid.messages['en-us'].Cancel + '</button>';
        } else {
            data.inlineEditing.editButton = '<button role="edit" class="bt-button-md"><i class="bt-icon pencil" /> ' + bt.grid.messages['en-us'].Edit.toUpperCase() + '</button>';
            data.inlineEditing.deleteButton = '<button role="delete" class="bt-button-md"><i class="bt-icon delete" /> ' + bt.grid.messages['en-us'].Delete.toUpperCase() + '</button>';
            data.inlineEditing.updateButton = '<button role="update" class="bt-button-md"><i class="bt-icon check-circle" /> ' + bt.grid.messages['en-us'].Update.toUpperCase() + '</button>';
            data.inlineEditing.cancelButton = '<button role="cancel" class="bt-button-md"><i class="bt-icon cancel" /> ' +bt.grid.messages['en-us'].Cancel.toUpperCase() + '</button>';
        }
    },
...
bt.grid.plugins.pagination = {
    ...
    private: {
        ...
        localizationBootstrap: function (data) {
            var msg = bt.grid.messages['en-us'];
...
bt.editor.methods = {
    ...
    localization: function (data) {
        var msg = bt.editor.messages['en-us'];
...
bt.datepicker.methods = {
    ...
    createCalendar: function ($datepicker, data) {
        ...
        date = bt.core.parseDate(data.value, data.format, 'en-us');
        ...
        if (data.footer) {
            ...
            $btnCancel = $('<button class="' + data.style.button + '">' + bt.core.messages['en-us'].cancel + '</button>');
            $btnCancel.on('click', function () { $datepicker.close(); });
            $footer.append($btnCancel);

            $btnOk = $('<button class="' + data.style.button + '">' + bt.core.messages['en-us'].ok + '</button>');
    ...
    renderHeader: function ($datepicker, $calendar, data, date) {
        ...
        if (data.header) {
            ...
            $year.html(bt.core.formatDate(date, 'yyyy', 'en-us'));
            $header.append($year);
            ...
            $date.html(bt.core.formatDate(date, 'ddd, mmm dd', 'en-us'));
            $header.append($date);
    ...
    updateHeader: function ($calendar, data, date) {
        $calendar.find('[role="header"] [role="year"]').removeClass('selected').html(bt.core.formatDate(date, 'yyyy', 'en-us'));
        $calendar.find('[role="header"] [role="date"]').addClass('selected').html(bt.core.formatDate(date, 'ddd, mmm dd', 'en-us'));
        $calendar.find('[role="header"] [role="hour"]').removeClass('selected').html(bt.core.formatDate(date, 'HH', 'en-us'));
        $calendar.find('[role="header"] [role="minute"]').removeClass('selected').html(bt.core.formatDate(date, 'MM', 'en-us'));
    },
    ...
    createNavigation: function ($datepicker, $body, $table, data) {
        ...
        for (i = data.weekStartDay; i < bt.core.messages['en-us'].weekDaysMin.length; i++) {
            $row.append('<th><div>' + bt.core.messages['en-us'].weekDaysMin[i] + '</div></th>');
        }
    ...
    renderMonth: function ($datepicker, $calendar, data) {
        ...
        $calendar.find('div[role="period"]').text(bt.core.messages['en-us'].monthNames[month] + ' ' + year);
    ...
    renderYear: function ($datepicker, $calendar, data) {
        ...
        for (i = 0; i < 3; i++) {
            $row = $('<tr />');
            for (m = (i * 4); m <= (i * 4) + 3; m++) {
                $month = $('<div>' + bt.core.messages['en-us'].monthShortNames[m] + '</div>');
    ...
    getMinDate: function (data) {
        ...
        if (data.minDate) {
            if (typeof (data.minDate) === 'string') {
                minDate = bt.core.parseDate(data.minDate, data.format, 'en-us');
            } else if (typeof (data.minDate) === 'function') {
                minDate = data.minDate();
                if (typeof minDate === 'string') {
                    minDate = bt.core.parseDate(minDate, data.format, 'en-us');
                }
    ...
    getMaxDate: function (data) {
        ...
        if (data.maxDate) {
            if (typeof data.maxDate === 'string') {
                maxDate = bt.core.parseDate(data.maxDate, data.format, 'en-us');
            } else if (typeof data.maxDate === 'function') {
                maxDate = data.maxDate();
                if (typeof maxDate === 'string') {
                    maxDate = bt.core.parseDate(maxDate, data.format, 'en-us');
                }
                ...
                if ($.isArray(data.disableDates)) {
                    for (i = 0; i < data.disableDates.length; i++) {
                        if (data.disableDates[i] instanceof Date && data.disableDates[i].getTime() === date.getTime()) {
                            result = false;
                        } else if (typeof data.disableDates[i] === 'string' && bt.core.parseDate(data.disableDates[i], data.format, 'en-us').getTime() === date.getTime()) {
                            result = false;
                        }
                    }
    ...
    change: function ($datepicker, $calendar, data, date) {
        var day = date.getDate(),
            ...
            value = bt.core.formatDate(date, data.format, 'en-us');
    ...
    value: function ($datepicker, value) {
        ...
        if (typeof (value) === "undefined") {
            ...
            date = bt.core.parseDate(value, data.format, 'en-us');
            if (date) {
                $calendar = $('body').find('[role="calendar"][guid="' + $datepicker.attr('data-guid') + '"]');
...
bt.timepicker.methods = {
    ...
    createClock: function ($timepicker) {
        ...
        if (data.header) {
            ...
            if (data.mode === 'ampm') {
                $mode.append($('<span role="am">' + bt.core.messages['en-us'].am + '</span>').on('click', function () {
                ...
                $mode.append('<br />');
                $mode.append($('<span role="pm">' + bt.core.messages['en-us'].pm + '</span>').on('click', function () {
                    var hour = bt.timepicker.methods.getHour($clock);
                    $clock.attr('mode', 'pm');
                    $(this).addClass('selected');
    ...
    open: function ($timepicker) {
        ...
        if ($timepicker.value()) {
            time = bt.core.parseDate($timepicker.value(), data.format, 'en-us');
        } else {
            time = new Date();
        }
        hour = time.getHours();
